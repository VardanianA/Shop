...
export const deleteData = async (req, res) => {
    OrderProduct.findByIdAndRemove(req.params.id, function (err, docs) {
        if (err) {
            res.status(500).json({ message: err.message })
        }
        else if (docs) {
...
export const updateData = async (req, res) => {
    OrderProduct.findByIdAndUpdate(req.params.id, req.body, (err, data) => {
        if (err) {
